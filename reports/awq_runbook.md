...
## Phase 6 Real AWQ Execution Path

Phase 6 extends `scripts/quantize_awq.py` from dry-run only into three mutually
exclusive modes:

- `--dry-run`: prints the planned AWQ W4A16 run and writes nothing.
- `--preflight`: validates paths and backend readiness before real execution.
- `--execute`: enters the real AutoAWQ execution path only when explicitly paired
  with `--confirm-write-artifacts`.
...
- `mode: preflight`
- backend readiness
- repository-relative model, calibration, and output paths
- `would_load_model: false`
- `would_write_artifacts: false`
- `performance_claim: not_benchmarked`

Real execution is implemented in
`src/optiz_qwen/compression/awq_execution.py`. Heavy imports for AutoAWQ and
`transformers` are delayed until `execute_autoawq_quantization()` after
`--execute` and `--confirm-write-artifacts` have both been supplied.

This local machine should run only tests and `--preflight`. Do not run
`--execute` here. Real `--execute` should be used only on a prepared Linux
server with a high-memory NVIDIA GPU, matching CUDA stack, AutoAWQ-compatible
dependencies, Qwen3.5-2B VLM weights, and a small calibration TSV.

The phase does not:

- install dependencies
- load model weights in `--dry-run` or `--preflight`
- read images
- write artifacts unless `--execute --confirm-write-artifacts` is used manually
- use the official competition evaluation dataset as a benchmark
- touch `benchmark_public.py`
- claim accuracy, TTFT, throughput, memory, or bandwidth gains

Artifacts generated by a future real run must stay under
`artifacts/quantized/qwen35_2b_awq_w4a16` or a subdirectory and must not be
committed to Git. Any metadata or summary from real execution must keep
`performance_claim: not_benchmarked` until a real BF16 baseline vs AWQ benchmark
artifact exists.
...
- Add guarded `--preflight` and `--execute` paths now.
- Keep the CLI dry-run only until the quantization server is available.

Evaluation dimensions:

- Preserves local safety by requiring explicit mode selection and artifact-write confirmation.
- Makes server-side readiness and path validation testable before using GPU time.
- Keeps heavyweight imports isolated from dry-run and preflight paths.
- Maintains fair-comparison boundaries around official benchmark code.

Final choice:

Add guarded execution plumbing now. Keeping dry-run only is weaker because it
