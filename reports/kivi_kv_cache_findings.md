...
| qserve_fused_kv | 实验链已落地 | Triton kernel 直接读取 packed KV；当前短上下文吞吐未胜出 |
...
3. `qserve_fused_kv` 已完成真实 CUDA 链路验证，但目前还不能宣称整体性能收益。

   1-sample 公平 greedy 对比中 TTFT 改善约 11.26%，吞吐下降约 10.16%。完整记录见
   `reports/qserve_fused_kv_decision.md`；10-sample 与 PPU 验证仍未完成。

## 结论

- 已完成：旧 KIVI 保留为可选链路，新 `qserve_kv` 作为另一条链路落地；
- 已实现：`qserve_fused_kv` packed-KV Triton 实验链，真实运行但短上下文吞吐未胜出；
- 当前胜出主线：视觉预算 36864 的 10-sample 结果同时改善 TTFT 和吞吐，准确率保持 1.0；
- 已验证：入口、调度、测试回归通过；
- 未完成：完整 public-dev 的 OCR/细粒度分类验证和 PPU 验证；
- 当前策略：视觉预算作为推荐候选，所有 KV 链继续保持可选对照。
